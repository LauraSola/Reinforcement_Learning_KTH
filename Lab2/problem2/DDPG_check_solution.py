...
try:
    ...
    model = torch.load(os.path.join(current_dir, 'outputs/neural-network-2-actor-224.89.pth'))
...
for i in EPISODES:
    ...
    while not (done or truncated):
        ...
        # if action.ndim == 0:  # If scalar, reshape into a 1D array
        #     action = np.array([action])

        # print("Action shape:", action.shape, "Action value:", action.detach().cpu().numpy())
